...
function Header() {
  ...
  return (
    <header
      ref={headerRef}
      className="sticky top-0 z-40 border-b border-border bg-surface shadow-[0_1px_0_rgba(19,35,62,0.04)]"
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-6 px-6">
        ...
        <nav className="hidden lg:flex lg:items-center lg:gap-1">
          {HEADER_NAV.map((group) => (
            <div key={group.label} className="relative">
              <button
                ...
                className={cn(
                  "flex items-center gap-1 rounded-[6px] px-3 py-2 font-sans text-sm font-medium text-text hover:bg-background",
              ...
              {openMenu === group.label && (
                <div className="absolute left-0 top-full mt-2 w-80 rounded-lg border border-border bg-surface p-2 shadow-[0_8px_24px_rgba(19,35,62,0.12)]">
          ...
          {HEADER_SIMPLE_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-[6px] px-3 py-2 font-sans text-sm font-medium text-text hover:bg-background"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <Link
            href="/login"
            className="font-sans text-sm font-medium text-text hover:text-secondary"
          >
            Log In
          </Link>
          <Link
