...
Phases 1–12 (all design phases) are approved. **Phase 13 (Development) is underway.** Milestones 1–7 (Project Setup, Design System, Public Layout, Homepage, Public Pages, Verification, Authentication) are approved; Milestone 8 (Applicant Portal) is built and delivered for approval. See `PROGRESS.md`'s Phase 13 Milestone Tracker for the full 17-milestone list and `docs/phases/phase-13-development-log.md` for full build detail per milestone.
...
**A recurring lesson worth carrying into Milestones 9–10** (Assessor/Admin portals, which will need the same sidebar + data-table patterns): when a Server Component needs to render a Client Component, watch for two distinct failure modes, both of which surfaced during Milestone 8 despite clean builds/typechecks — (1) passing anything function-shaped (icons, `DataTable` column `render` callbacks) as a prop from server to client, and (2) importing a plain data constant *from* a `"use client"` file *into* server code. Both fail at runtime, not build time, and (2) doesn't even get caught by TypeScript. The fix for both is the same: keep the data plain and framework-agnostic in a file with no `"use client"` directive, and let the client component that needs icons/render-functions define them locally instead of receiving them as props from a server parent.
...
- **Real Applicant Portal built (Milestone 8)**: `/portal/applicant/{dashboard,applications,applications/[id],invoices,invoices/[id],accreditation,messages,profile,security}` — all functional against a placeholder per-user data store, with real Server Actions for document upload, messaging, application drafting/submission, and password change. **Assessor and Admin dashboards are still placeholder pages — Milestones 9–10.**
- The entire public site, auth flow, and Applicant portal now resolve — only Assessor/Admin portals remain unbuilt.
