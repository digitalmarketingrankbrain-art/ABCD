...
- **Still in-memory placeholder stores** (a deliberate, documented scope boundary from Milestone 12, not an oversight): Applications, Assignments, Invoices, Messages, Competence, Availability, Verification records. They reset on server restart. Migrating these to Postgres would follow the exact same pattern used for Documents/Notifications if picked back up.
